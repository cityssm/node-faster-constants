...
await describe('faster-constants', async () => {
  await it('should import "inventory" constants in multiple ways', () => {
    ...
    assert.strictEqual(
      constants.inventory.items.itemName.inputId,
  ...
  await it('should import "other" constants in multiple ways', () => {
    ...
    assert.strictEqual(
      constants.other.eslint.cspellWords[0],
  ...
  await it('should import "vendors" constants in multiple ways', () => {
    ...
    assert.strictEqual(
      constants.vendors.vendors.vendorName.maxLength,
      vendorConstants.vendors.vendorName.maxLength
    )
    
    assert.strictEqual(
      constants.vendors.vendors.vendorName.maxLength,
      vendorVendorConstants.vendorName.maxLength
    )
  })
})
